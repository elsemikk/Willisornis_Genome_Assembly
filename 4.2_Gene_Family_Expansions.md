...
```bash
#set up environment
mkdir /home/0_GENOMES1/0_WEIRLAB_GENOMES_CHROMIUMX/anteater/Cafe
cd /home/0_GENOMES1/0_WEIRLAB_GENOMES_CHROMIUMX/anteater/Cafe
...
#Get list of families that have expanded in anteater, whether or not significant
...
wc -l *_sig_changes_large_* #Thamno: 0, Rhemel: 3, Hypoch: 2, Anteater: 3
...
```bash
mkdir /home/0_GENOMES1/0_WEIRLAB_GENOMES_CHROMIUMX/anteater/Cafe/expanded
cd /home/0_GENOMES1/0_WEIRLAB_GENOMES_CHROMIUMX/anteater/Cafe/expanded
...
#Get a list of the anteater genes in these orthogroups
...
#extract GO terms
grep -P '\tgene\t' Anteater.standardplus.functional_5interpro.gff | sed 's/maker.*ID=//g' | sed 's/;Name=.*Dbxref=/\t/g' | sed 's/;Ontology_term=/\t/g' | sed 's/;Name=.*$//g' > Gene_mappings.txt

mkdir /home/0_GENOMES1/0_WEIRLAB_GENOMES_CHROMIUMX/anteater/Cafe/enrichment_analysis
cd /home/0_GENOMES1/0_WEIRLAB_GENOMES_CHROMIUMX/anteater/Cafe/enrichment_analysis

awk 'BEGIN{FS="\t"}{print $2,$2,$2,$4}' ~/anteater/functional_ann/Gene_mappings.txt > GO_mappings.ermineJ.txt
...
#this is a bad  way
for taxon in ant Rhemel Hypoch Thamno ; do awk 'BEGIN{FS="\t"}{print $2,"0"}' ~/anteater/functional_ann/Gene_mappings.txt > "$taxon"_expanded_genesets ; done
for taxon in ant Rhemel Hypoch Thamno ; do cat /home/0_GENOMES1/0_WEIRLAB_GENOMES_CHROMIUMX/anteater/Cafe/expanded/"$taxon"_expanded_geneIDs.txt | sed 's/ .*$//g' | while read gene ; do sed -i "s/$gene 0/$gene 1/g" "$taxon"_expanded_genesets ; done ; done
...
#awk 'BEGIN{FS="\t"}{print $2,"0"}' ~/anteater/functional_ann/Gene_mappings.txt > contracted_genesets
#cat /home/0_GENOMES1/0_WEIRLAB_GENOMES_CHROMIUMX/anteater/Cafe/contracted/contracted_geneIDs.txt | while read gene ; do sed -i "s/$gene 0/$gene 1/g" contracted_genesets ; done
...
Alternately, rather than focus on just the significant ones, we can do GO analysis on all of the genes.

Now a ranked list regardless of significance
```bash
cd /home/0_GENOMES1/0_WEIRLAB_GENOMES_CHROMIUMX/anteater/Cafe/analysis
...
setwd("/Users/elsemikkelsen/Desktop/Anteater/orthogroups") #change to the directory containing the tree and orthogroup data
