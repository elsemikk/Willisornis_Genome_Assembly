...
mkdir ~/anteater/functional_ann
cd ~/anteater/functional_ann

#search for hits in the databases with Interproscan

#Pfam
interproscan.sh -iprlookup -goterms -appl Pfam-32.0 -i /home/else/anteater/maker/maker_R4/Anteater_input.all.maker.proteins.fasta -b Anteater_R4prots -f tsv
#PANTHER
time interproscan.sh -iprlookup -goterms -appl PANTHER-14.1 -i /hhome/else/anteater/maker/maker_R4/Anteater_input.all.maker.proteins.fasta -b Anteater_R4protsPANTHER -f tsv
#Others
time interproscan.sh -iprlookup -goterms -appl PRINTS-42.0 -i /hhome/else/anteater/maker/maker_R4/Anteater_input.all.maker.proteins.fasta -b Anteater_R4protsPrints -f tsv
time interproscan.sh -iprlookup -goterms -appl ProSiteProfiles-2019_01 -i /hhome/else/anteater/maker/maker_R4/Anteater_input.all.maker.proteins.fasta -b Anteater_R4protsProsite -f tsv
time interproscan.sh -iprlookup -goterms -appl SMART-7.1 -i /hhome/else/anteater/maker/maker_R4/Anteater_input.all.maker.proteins.fasta -b Anteater_R4protsSMART -f tsv
...
#interproscan.sh -iprlookup -goterms -i /hhome/else/anteater/maker/maker_R4/Anteater_input.all.maker.proteins.fasta -b Anteater_R4prots_allinterpro -f tsv

#concatenate all 5 interpro results if done separately
cat Anteater_R4prot*.tsv > Anteater_R4prot_5interpro.tsv

#add the interpro results to the maker gff
ipr_update_gff /hhome/else/anteater/maker/maker_R4/Anteater_rnd4.all.maker.gff Anteater_R4prot_5interpro.tsv > Anteater.max.functional_5interpro.gff

#filter to retain only best genes, compare two filtering options:
perl /home/0_PROGRAMS/Genome_annotation/quality_filter.pl -d Anteater.max.functional_5interpro.gff > Anteater.default.functional_5interpro.gff #default build: only proteins with RNA/protein evidence (AED<1)

perl /home/0_PROGRAMS/Genome_annotation/quality_filter.pl -s Anteater.max.functional_5interpro.gff > Anteater.standard.functional_5interpro.gff #standard build: all proteins with evidence (AED<1) and/or a functional annotation (Pfam)

perl /home/0_PROGRAMS/Genome_annotation/quality_filter_PfamPANTHER.pl -s Anteater.max.functional_5interpro.gff > Anteater.standardplus.functional_5interpro.gff #standard build: all proteins with evidence (AED<1) and/or a functional annotation (Pfam or PANTHER)

#count number of genes
grep -cP '\tgene\t' /hhome/else/anteater/maker/maker_R4/Anteater_rnd4.all.maker.gff #number of genes originally
grep -cP '\tgene\t' Anteater.default.functional_5interpro.gff #number genes with evidence
grep -cP '\tgene\t' Anteater.standard.functional_5interpro.gff #number genes with evidence OR Pfam domain
grep -cP '\tgene\t' Anteater.standardplus.functional_5interpro.gff #number genes with evidence OR Pfam domain or PANTHER
less Anteater_R4prots.tsv | cut -f 1 | sort | uniq | wc -l #number of genes with Pfam annotations
less Anteater_R4protsPANTHER.tsv | cut -f 1 | sort | uniq | wc -l #number of genes with PANTHER annotations
less Anteater_R4protsPrints.tsv | cut -f 1 | sort | uniq | wc -l #number of genes with PRINTS annotations
less Anteater_R4protsProsite.tsv | cut -f 1 | sort | uniq | wc -l #number of genes with Prosite annotations
less Anteater_R4protsSMART.tsv | cut -f 1 | sort | uniq | wc -l #number of genes with Pfam annotations
less Anteater_R4prot_5interpro.tsv | cut -f 1 | sort | uniq | wc -l #number of genes with annotations
```
Interproscan
...
#AGAT stats
conda activate agat
agat_sp_functional_statistics.pl --gff Anteater.standardplus.functional_5interpro.gff -o Anteater.standardplus.functional_5interpro.stats
```
...
maker_map_ids --prefix WilPoe --justify 5 Anteater.standardplus.functional_5interpro.gff > Anteater.standardplus.name.map
# replace names in GFF files
map_gff_ids Anteater.standardplus.name.map Anteater.standardplus.functional_5interpro.gff
# replace names in FASTA headers
cp /hhome/else/anteater/maker/maker_R4/Anteater_input.all.maker.proteins.fasta .
map_fasta_ids Anteater.standardplus.name.map Anteater_input.all.maker.proteins.fasta
```
...
#agat_sp_extract_sequences.pl --protein --gff Anteater.standardplus.functional_5interpro.gff -f /home/0_GENOMES1/0_WEIRLAB_GENOMES_CHROMIUMX/anteater/genome/anteater700.fasta -o Anteater_standardproteins.fasta #did not work, complained that "Got a sequence without letters. Could not guess alphabet"

#Use seqtk to filter a fasta based on a list
cat Anteater.standardplus.name.map | cut -f 2 | sort | uniq > Anteater.standardplus_genelist #extract list of genes to keep
/home/0_PROGRAMS/seqtk/seqtk subseq Anteater_input.all.maker.proteins.fasta Anteater.standardplus_genelist > Anteater.standardplus.proteins.fasta
...
cp /home/0_PROGRAMS/busco/config/config.ini ./anteater_busco_config.ini
python /home/0_PROGRAMS/busco/scripts/run_BUSCO.py -i Anteater.standardplus.proteins.fasta  -o Anteater.standardplus.proteins -l /home/0_PROGRAMS/busco/aves_odb9 -m proteins -c 24 -sp chicken -z --augustus_parameters='--progress=true'
python /home/0_PROGRAMS/busco/scripts/run_BUSCO.py -i Anteater_input.all.maker.proteins.fasta -o Anteater.all.proteins -l /home/0_PROGRAMS/busco/aves_odb9 -m proteins -c 24 -sp chicken -z --augustus_parameters='--progress=true'
```
...
Anteater.standardplus.proteins.fasta|17,475 |93% |C:80.1% (3937)|S:78.7%|D:1.4 (69)|F:12.9% (634)|M:7.0% (344)|202.1|
...
First, we need to get the Swissprot/Uniprot database and make a blast database for it. I already downloaded this database in the [proteomes](https://github.com/elsemikk/Anteater_Genome/blob/master/Proteomes.md) section.  
Then, we will BLAST our proteome against this BLAST database to see what our proteins match.
```bash
#setup environment
cd /home/else/anteater/functional_ann
...
blastp -db /home/0_GENOMES1/0_Genetic_resources/Uniprot_swissprot/uniprot_sprot.fasta -query Anteater.standardplus.proteins.fasta -out maker2uni.blastp -evalue .000001 -outfmt 6 -num_alignments 1 -seg yes -soft_masking true -lcase_masking -max_hsps 1 -num_threads 23

#add the homology results to the FASTA file
maker_functional_fasta /home/0_GENOMES1/0_Genetic_resources/Uniprot_swissprot/uniprot_sprot.fasta maker2uni.blastp Anteater.standardplus.proteins.fasta > Anteater.standardplus.SPproteins.fasta
...
blastp -db /home/0_GENOMES1/0_Genetic_resources/Uniprot_trembl/Trembl_birds_Dec2019.fasta -query Anteater.standardplus.proteins.fasta -out maker2trem.blastp -evalue .000001 -outfmt 6 -num_alignments 1 -seg yes -soft_masking true -lcase_masking -max_hsps 1 -num_threads 23

#add the homology results to the FASTA file
maker_functional_fasta /home/0_GENOMES1/0_Genetic_resources/Uniprot_trembl/Trembl_birds_Dec2019.fasta maker2trem.blastp Anteater.standardplus.proteins.fasta > Anteater.standardplus.Trproteins.fasta
```
...
#add the homology results to the FASTA file
maker_functional_fasta /home/0_GENOMES1/0_Genetic_resources/Uniprot_cat_Tremblbirds.temp nr_blastp_for_naming Anteater.standardplus.proteins.fasta > Anteater.standardplus.namedproteins.fasta

#finally, get the transcript data as well and name the same way
cp /home/else/anteater/maker/maker_R4/Anteater_input.all.maker.transcripts.fasta .
map_fasta_ids Anteater.standardplus.name.map Anteater_input.all.maker.transcripts.fasta
/home/0_PROGRAMS/seqtk/seqtk subseq Anteater_input.all.maker.transcripts.fasta Anteater.standardplus_genelist > Anteater.standardplus.transcripts.fasta
maker_functional_fasta /home/0_GENOMES1/0_Genetic_resources/Uniprot_cat_Tremblbirds.temp nr_blastp_for_naming Anteater.standardplus.transcripts.fasta > Anteater.standardplus.namedtranscripts.fasta
